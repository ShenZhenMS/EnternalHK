Public Class DIServer_StockOut : Inherits CPSLIB.DIServer.Document
    ...
    Public Sub New(ByVal _Setting As Settings, ByVal _diConn As CPSLIB.DIServer.DIServerConnection)
        MyBase.New(_diConn, SAPbobsCOM.BoObjectTypes.oInventoryGenExit)
    ...
    Public Function Start(ByVal _o As String) As Boolean
        ...
        Try
            ...
            If _drRow.Length > 0 Then
                NewDocument()
                If _isDraft Then
                    SetValue(FLD_HDR_DocObjectCode, "60")
                End If

                SetValue(FLD_HDR_DocDate, Convert.ToDateTime(_drRow(0)(Inventory_Inout.Fld_DocDate)).ToString("yyyyMMdd"))
                SetValue(FLD_HDR_Series, _SAPInventory.DocSeries(DIServer_StockOut.ObjType, Convert.ToDateTime(_drRow(0)(Inventory_Inout.Fld_DocDate)).ToString("yyyyMMdd"), _drRow(0)(Inventory_Inout.Fld_DocSeries)))
                ...
                For Each dr In _drRow
                    ...
                    _GLAccount = _SAPInventory.GLAccount(DIServer_StockOut.ObjType,
                                                         Convert.ToDateTime(_drRow(0)(Inventory_Inout.Fld_DocDate)).ToString("yyyyMMdd"),
                                                         dr(Inventory_Inout.Fld_DocSeries), dr(Inventory_Inout.Fld_ItemCode))
                    setRowsValue(FLD_DTL_AccountCode, _GLAccount)
                    _htCOADefaultDr = _SAPInventory.GetCOADefaultDR(_GLAccount)

                    For Each o As Object In _htCOADefaultDr.Keys
                        _Debug.Write(o)
                        _Debug.Write(_htCOADefaultDr(o))
                    Next
