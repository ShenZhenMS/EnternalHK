Public Class WS_StockIn : Inherits DIServer
    ...
    '    Dim _msgHeader As InventoryReceive.MsgHeader
    '    Dim _StockIn As InventoryReceive.InventoryGenEntryService

    '    Dim oDoc As InventoryReceive.Document
    '    Dim docLine As InventoryReceive.DocumentDocumentLine
    '    Dim docLineBatch As InventoryReceive.DocumentDocumentLineBatchNumber
    ...
    '            _StockIn = New InventoryReceive.InventoryGenEntryService
    '            oDoc = New InventoryReceive.Document

    '            _msgHeader = New InventoryReceive.MsgHeader
    '            _msgHeader.SessionID = MyBase.SessionID
    '            _msgHeader.ServiceName = InventoryReceive.MsgHeaderServiceName.InventoryGenEntryService
    ...
    '                    End If
    '                     docLine = New InventoryReceive.DocumentDocumentLine

    '                End If

    '                docLineBatch = Nothing
    '                docLineBatch = New InventoryReceive.DocumentDocumentLineBatchNumber
    '                If IsDBNull(dr(InventoryInoutForWMS.Fld_BatchNum)) = False Then
    '                    docLineBatch.BatchNumber = dr(Inventory_Inout.Fld_BatchNum) * _SAPInventory.GetSalesItemPerBaseUnit(dr(Inventory_Inout.Fld_ItemCode))
    '                    docLineBatch.QuantitySpecified = True
    '                    docLineBatch.Quantity = dr(Inventory_Inout.Fld_Quantity)
    ...
    '                docLine.AccountCode = _SAPInventory.GLAccount(WS_StockIn.ObjType, oDoc.DocDate.ToString("yyyyMMdd"), dr(Inventory_Inout.Fld_DocSeries))
    ...
    '            oDoc.DocumentLines = InventoryReceiptDocumentLineArray(_alDocumentLine)
    ...
    Public Function StartDraft(ByVal _o As String, ByVal dataTb As DataTable) As Boolean
        Me._dt = dataTb
        ...
        Try
            ...
            If _htKeyValue.Count > 0 Then
                For Each o As Object In _htKeyValue.Keys
                    ...
                    Else
                        _htDocStatus(o) = StartDraft(o, _dt)
                    End If
