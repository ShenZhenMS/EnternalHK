Public Class DIServer_SalesInvoice : Inherits CPSLIB.DIServer.Document
    ...
    Dim Fld_LINE_LineNum As String = "LineNum"
    ...
    Public Sub New(ByVal _Setting As Settings, ByVal _diConnection As CPSLIB.DIServer.DIServerConnection)
        MyBase.New(_diConnection, SAPbobsCOM.BoObjectTypes.oInvoices)
    ...
    Public Function Create(ByVal _DocEntry As String, ByVal _SOEntry As String, ByVal _CardCode As String, ByVal _allowbackdate As Boolean) As Boolean
        ...
        mSql = String.Format("Select * From DLN1 Where DocEntry = '{0}' ", _DocEntry)

        _dt_DocEntry = SAPConnection.ExecuteDatatable(mSql)


        NewDocument()

        SetValue(Fld_HDR_CardCode, _CardCode)
        MyBase.setUDF("ODLN", _DocEntry)
        If _allowbackdate Then
            SetValue(Fld_HDR_DocDate, SAPConnection.GetSODocDate(_SOEntry).ToString("yyyyMMdd"))
        Else
        ...
        Try
            ...
            For i As Integer = 0 To _dt_DocEntry.Rows.Count - 1
                ...
                setUDF("DLN1", _DocEntry, _dt_DocEntry.Rows(i).Item(Fld_LINE_LineNum))
                setRowsValue(Fld_LINE_BaseType, "15")
                setRowsValue(Fld_LINE_BaseEntry, _DocEntry)
                setRowsValue(Fld_LINE_BaseLine, _dt_DocEntry.Rows(i).Item(Fld_LINE_LineNum))
                AddRow()
            ...
            If MyBase.Post(Command.AddObject) = CommandStatus.Fail Then
                _Sales.UpdateErrorStatus(_SOEntry, "-1", MyBase.CmdMessage)
                _ret = False
            Else

                _ret = True
                _Sales.UpdateSuccessStatus(_SOEntry, NewEntry, String.Empty)
        ...
        Catch ex As Exception
            _Sales.UpdateErrorStatus(_SOEntry, "-1", ex.Message)
            _ret = False
        End Try
